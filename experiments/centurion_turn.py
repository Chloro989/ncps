...
# 特徴の正規化。Phase 10 の第1回で得た実測応答112件から決めた
# (results/centurion_turn_responses.txt)。進みだけは設計値。
#
# 第1回は想像で [0.5, 1.0, 1.0, 0.7] / [0.3, 1.0, 0.4, 0.2] と置いていた。
# 実測との差は、轍語率が -0.75σ・豊かさが +1.09σ・長さが +0.63σ。
# 平均が約1σずれ、しかも変動は想定の半分以下だった。
# この状態では偏差の大部分が毎ターン同じ定数になり、順位がそれに固定される。
# 実際、回路は9句のうち「断定を恐れず」を選択の92%に入れ、3句は一度も
# 使わなかった。基準を引くだけでは足りず、基準の位置が実測と合っている
# 必要がある。Phase 0 の教訓の再来 — 回路は入力を読めていなかった
FEATURE_MEAN = [0.5, 0.28, 1.27, 0.91]
FEATURE_STD = [0.3, 0.44, 0.27, 0.09]
